...
public sealed class MultiplayerScreen : IScreen
{
    ...
    public void OnResize(Rectangle viewport)
    {
        ...
        var panelW = Math.Min(1120, viewport.Width - 64);
        var panelH = Math.Min(700, viewport.Height - 96);
        ...
        var pad = 20;
        var headerH = _font.LineHeight + 12;
        _infoRect = new Rectangle(panelX + pad, panelY + pad, panelW - pad * 2, headerH);

        var buttonRowH = _font.LineHeight + 16;
        _buttonRowRect = new Rectangle(panelX + pad, panelY + panelH - pad - buttonRowH, panelW - pad * 2, buttonRowH);
        _listRect = new Rectangle(panelX + pad, _infoRect.Bottom + 10, panelW - pad * 2, _buttonRowRect.Top - _infoRect.Bottom - 18);

        var listHeaderH = _font.LineHeight + 18;
        _tabLanRect = new Rectangle(_listRect.X + 8, _listRect.Y + 6, (_listRect.Width - 24) / 2, _font.LineHeight + 10);
        _tabOnlineRect = new Rectangle(_tabLanRect.Right + 4, _tabLanRect.Y, (_listRect.Width - 12) / 2, _tabLanRect.Height);
        _listBodyRect = new Rectangle(_listRect.X + 4, _listRect.Y + listHeaderH, _listRect.Width - 8, Math.Max(0, _listRect.Height - listHeaderH - 4));
        _playerNameInputRect = new Rectangle(
            _listBodyRect.X + 10,
            _listBodyRect.Y + _font.LineHeight + 24,
            Math.Max(220, Math.Min(420, _listBodyRect.Width - 210)),
            _font.LineHeight + 12);
        var saveX = _playerNameInputRect.Right + 8;
        var saveW = Math.Max(110, Math.Min(180, _listBodyRect.Right - saveX));
        _saveNameBtn.Bounds = new Rectangle(saveX, _playerNameInputRect.Y, saveW, _playerNameInputRect.Height);
        LayoutActionButtons();
    ...
    private void DrawTabs(SpriteBatch sb)
    {
        ...
        _font.DrawString(sb, lanText, new Vector2(_tabLanRect.Center.X - lanSize.X / 2f, _tabLanRect.Center.Y - lanSize.Y / 2f), Color.White);
        _font.DrawString(sb, onText, new Vector2(_tabOnlineRect.Center.X - onSize.X / 2f, _tabOnlineRect.Center.Y - onSize.Y / 2f), Color.White);
    }
    ...
    private void DrawOnlineInfo(SpriteBatch sb)
    {
        ...
        y += _font.LineHeight + 2;
        _font.DrawString(sb, "Friends: click FRIENDS to add or accept requests.", new Vector2(x, y), Color.White);
        y += _font.LineHeight + 10;
    ...
    {
        const int gap = 12;
        var rowY = _buttonRowRect.Y;
        var rowH = _buttonRowRect.Height;
        ...
        var lanCount = 3;
        var lanButtonW = Math.Max(120, (_buttonRowRect.Width - gap * (lanCount - 1)) / lanCount);
        var lanX = _buttonRowRect.X;
        _refreshBtn.Bounds = new Rectangle(lanX, rowY, lanButtonW, rowH);
        lanX += lanButtonW + gap;
        _hostBtn.Bounds = new Rectangle(lanX, rowY, lanButtonW, rowH);
        lanX += lanButtonW + gap;
        _joinBtn.Bounds = new Rectangle(lanX, rowY, lanButtonW, rowH);
        _yourIdBtn.Bounds = Rectangle.Empty;
        _friendsBtn.Bounds = Rectangle.Empty;
    }
