...
public sealed class GameWorldScreen : IScreen, IMouseCaptureScreen
{
    ...
    private readonly Dictionary<string, PendingJoinOverlayRequest> _pendingJoinOverlayRequests = new(StringComparer.OrdinalIgnoreCase);
    ...
    private void ResumeGameplayAfterHosting()
    {
        ...
        _pendingJoinOverlayRequests.Clear();
    }
    ...
    private void UpdateNetwork(float dt)
    {
        ...
        if (_lanSession is EosP2PHostSession eosHost)
        {
            ...
            ProcessPendingJoinRequests(eosHost);
        }
    ...
    private void ProcessPendingJoinRequests(EosP2PHostSession hostSession)
    {
        while (hostSession.TryDequeueJoinRequest(out var request))
        {
            ...
            if (_pendingJoinOverlayRequests.ContainsKey(peerId))
                continue;

            var displayName = string.IsNullOrWhiteSpace(request.DisplayName)
                ? $"Player {PlayerProfile.ShortId(peerId)}"
                : request.DisplayName.Trim();
            _pendingJoinOverlayRequests[peerId] = new PendingJoinOverlayRequest
            {
                ProductUserId = peerId,
                DisplayName = displayName
            };

            AddJoinRequestChatPrompt(peerId, displayName);
        }
    }
    ...
    private void AddJoinRequestChatPrompt(string peerId, string displayName)
    {
        ...
        AddChatLine(
            $"Decline {displayName}",
            isSystem: true,
            hoverText: "Decline this join request.",
            actionLabel: "DECLINE",
            customActionToken: JoinDeclineActionPrefix + peerId,
            customActionStatus: $"{displayName} declined.");
    }
    ...
    {
        if (_lanSession == null || !_lanSession.IsHost || string.IsNullOrWhiteSpace(_worldPath) || _remotePlayers.Count == 0)
            return 0;
        ...
        foreach (var pair in _remotePlayers)
        {
            ...
            var model = pair.Value;
            var username = NormalizeIdentityToken(ResolvePlayerName(playerId));
    ...
    private void ExecuteCustomChatAction(string token, string statusText)
    {
        ...
        if (token.StartsWith(JoinApproveActionPrefix, StringComparison.Ordinal))
        {
            ...
            if (!hostSession.ApproveJoinRequest(peerId))
            {
                ...
                _pendingJoinOverlayRequests.Remove(peerId);
                return;
            }

            if (_pendingJoinOverlayRequests.TryGetValue(peerId, out var pending))
            {
                _profile.AddOrUpdateFriend(pending.ProductUserId, pending.DisplayName);
                _profile.Save(_log);
            }

            _pendingJoinOverlayRequests.Remove(peerId);
            var message = string.IsNullOrWhiteSpace(statusText) ? "Join approved." : statusText;
            SetCommandStatus(message, 4f, echoToChat: false);
            return;
        }
        ...
        if (token.StartsWith(JoinDeclineActionPrefix, StringComparison.Ordinal))
        {
            ...
            _pendingJoinOverlayRequests.Remove(peerId);
            var message = string.IsNullOrWhiteSpace(statusText) ? "Join declined." : statusText;
            SetCommandStatus(message, 4f, echoToChat: false);
        }
    }
    ...
    private void BuildCommandCompletionCandidates(in CommandCompletionContext context, List<CommandCompletionCandidate> output)
    {
        ...
        if (!_commandLookup.TryGetValue(context.Tokens[0], out var command))
            return;
        if (!HasCommandPermission(command.Permission))
            return;
        ...
        string? customActionStatus = null)
    {
        ...
        if (_chatLines.Count > 0 && string.Equals(_chatLines[^1].Text, text, StringComparison.Ordinal))
        {
            ...
            if (hasTeleportAction || hasCopyAction || hasCustomAction)
    ...
    private void EnsureCommandRegistry()
    {
        ...
        RegisterCommand("gamemode", new[] { "gm" }, "/gamemode <artificer|veilwalker|veilseer|gui>", "Change gamemode or open selector.", CommandPermission.Operator, ExecuteGameModeCommand);
        RegisterCommand("artificer", Array.Empty<string>(), "/artificer", "Set ARTIFICER mode.", CommandPermission.Operator, _ => ExecuteDirectGameMode(GameMode.Artificer));
        RegisterCommand("veilwalker", Array.Empty<string>(), "/veilwalker", "Set VEILWALKER mode.", CommandPermission.Operator, _ => ExecuteDirectGameMode(GameMode.Veilwalker));
        RegisterCommand("veilseer", Array.Empty<string>(), "/veilseer", "Set VEILSEER mode.", CommandPermission.Operator, _ => ExecuteDirectGameMode(GameMode.Veilseer));
    ...
    private struct PendingJoinOverlayRequest
    {
