...
public sealed class JoinByCodeScreen : IScreen
{
    ...
    private Dictionary<string, EosFriendSnapshot>? _pendingPresenceByJoinCode;
    ...
    public void OnResize(Rectangle viewport)
    {
        ...
        var panelW = Math.Min(720, viewport.Width - 80);
		var panelH = Math.Min(560, viewport.Height - 110);
		...
		var friendsH = Math.Max(120, _panelRect.Bottom - pad - buttonRowH - 12 - friendsY);
    ...
    private bool TryNormalizeHostCode(string rawInput, out string hostCode, out string error)
    {
        ...
        if (value.StartsWith("RC-", StringComparison.OrdinalIgnoreCase))
        {
            ...
            if (resolved == null)
            {
                error = "Friend code not found in saved friends.";
                return false;
            }
    ...
    private async Task RefreshFriendPresenceAsync()
    {
        ...
        try
        {
            ...
            var refreshed = new Dictionary<string, EosFriendSnapshot>(StringComparer.OrdinalIgnoreCase);
            for (var i = 0; i < snapshots.Count; i++)
            {
                ...
                refreshed[joinCode] = snapshots[i];
            }
    ...
    {
        Dictionary<string, EosFriendSnapshot>? pending;
        lock (_presenceApplyLock)
        {
        ...
        foreach (var friend in _profile.Friends)
        {
            ...
            if (!string.IsNullOrWhiteSpace(snap.DisplayName)
                && !string.Equals(friend.LastKnownDisplayName, snap.DisplayName, StringComparison.Ordinal))
            {
