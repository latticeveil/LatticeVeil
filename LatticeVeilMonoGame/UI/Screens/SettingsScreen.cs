...
public sealed class OptionsScreen : IScreen
{
    ...
    private static readonly (string value, string label)[] UiFontOptions = new[]
    {
        (PixelFont.ClassicStyle, "CLASSIC"),
        (PixelFont.TerminalStyle, "TERMINAL")
    };
    private static readonly string[] UiFontLabels = UiFontOptions.Select(o => o.label).ToArray();
    ...
    private Checkbox _soulMarkerGuidanceEnabled;
    private Rectangle _uiFontBox;
    private bool _uiFontOpen;
    ...
    private Rectangle _unsavedModalRect;
    private Rectangle _unsavedModalListRect;
    private Rectangle _unsavedStayRect;
    private Rectangle _unsavedDiscardRect;
    private Rectangle _unsavedApplyRect;
    private bool _showUnsavedChangesModal;
    private float _unsavedChangesScroll;
    private int _unsavedChangesContentHeight;
    private List<SettingChangeEntry> _pendingSettingChanges = new();

    private sealed class SettingChangeEntry
    {
    ...
    public OptionsScreen(MenuStack menus, AssetLoader assets, PixelFont font, Texture2D pixel, Logger log, global::Microsoft.Xna.Framework.GraphicsDeviceManager graphics)
    {
        ...
        _soulMarkerGuidanceEnabled = new Checkbox("SOUL MARKER GUIDE", _working.SoulDeathMarkerGuidanceEnabled, v =>
        {
            _working.SoulDeathMarkerGuidanceEnabled = v;
            _log.Info($"Option changed: SoulDeathMarkerGuidanceEnabled = {v}");
        });
    ...
    public void OnResize(Rectangle viewport)
    {
        ...
        var modalWidth = Math.Min((int)Math.Round(_panelRect.Width * 0.7f), 860);
        var modalHeight = Math.Min((int)Math.Round(_panelRect.Height * 0.74f), 620);
        _unsavedModalRect = new Rectangle(
            _viewport.Center.X - modalWidth / 2,
            _viewport.Center.Y - modalHeight / 2,
            modalWidth,
            modalHeight);
        _unsavedModalListRect = new Rectangle(
            _unsavedModalRect.X + 28,
            _unsavedModalRect.Y + 108,
            _unsavedModalRect.Width - 56,
            Math.Max(120, _unsavedModalRect.Height - 208));
        var modalButtonWidth = Math.Max(150, (_unsavedModalRect.Width - 76) / 3);
        var modalButtonHeight = 46;
        var modalButtonY = _unsavedModalRect.Bottom - modalButtonHeight - 22;
        _unsavedStayRect = new Rectangle(_unsavedModalRect.X + 18, modalButtonY, modalButtonWidth, modalButtonHeight);
        _unsavedDiscardRect = new Rectangle(_unsavedStayRect.Right + 10, modalButtonY, modalButtonWidth, modalButtonHeight);
        _unsavedApplyRect = new Rectangle(_unsavedDiscardRect.Right + 10, modalButtonY, modalButtonWidth, modalButtonHeight);
        ...
        _uiFontBox = new Rectangle(controlsValueX, reticleTop + 62, controlsValueWidth, dropdownHeight);
        _reticleStyleBox = new Rectangle(controlsValueX, reticleTop + 122, controlsValueWidth, dropdownHeight);
        _reticleColorBox = new Rectangle(controlsValueX, reticleTop + 182, controlsValueWidth, dropdownHeight);
        _blockOutlineColorBox = new Rectangle(controlsValueX, reticleTop + 242, controlsValueWidth, dropdownHeight);
        _flyingOutlineEnabled.Bounds = new Rectangle(controlsValueX, reticleTop + 302, controlsValueWidth, 42);
        _flyingOutlineColorBox = new Rectangle(controlsValueX, reticleTop + 362, controlsValueWidth, dropdownHeight);
        _notificationModeBox = new Rectangle(controlsValueX, reticleTop + 422, controlsValueWidth, dropdownHeight);
        _indicatorsEnabled.Bounds = new Rectangle(controlsValueX, reticleTop + 482, controlsValueWidth, 42);
        _soulMarkerGuidanceEnabled.Bounds = new Rectangle(controlsValueX, reticleTop + 542, controlsValueWidth, 42);
        _nametagModeBox = new Rectangle(controlsValueX, reticleTop + 602, controlsValueWidth, dropdownHeight);
        _nametagFadeSeconds.Bounds = new Rectangle(controlsValueX, reticleTop + 662, controlsValueWidth, sliderHeight);
        _reticleSize.Bounds = new Rectangle(controlsValueX, reticleTop + 722, controlsValueWidth, sliderHeight);
        _reticleThickness.Bounds = new Rectangle(controlsValueX, reticleTop + 782, controlsValueWidth, sliderHeight);
    ...
    public void Update(GameTime gameTime, InputState input)
    {
        ...
        {
            UpdateUnsavedChangesModal(input);
            return;
        }
        ...
        switch (_tab)
        {
            ...
            case Tab.Controls:
                ...
                else if (_controlsSubTab == ControlsSubTab.Interface)
                {
                    ...
                    UpdateCheckboxWithScroll(_soulMarkerGuidanceEnabled, input);
                    UpdateCheckboxWithScroll(_flyingOutlineEnabled, input);
                    UpdateUiFontDropdown(input);
    ...
    private void UpdateUiFontDropdown(InputState input)
    {
        var scroll = GetScrollOffset();
        var box = ScrollRect(_uiFontBox, scroll);
        UpdateSimpleDropdown(input, box, ref _uiFontOpen, UiFontLabels, idx =>
        {
            _working.UiFontStyle = UiFontOptions[idx].value;
            _font.SetStyle(_working.UiFontStyle);
            _log.Info($"Option changed: UiFontStyle = {_working.UiFontStyle}");
        });
    }
    ...
    private void DrawControls(SpriteBatch sb)
    {
        ...
        else if (_controlsSubTab == ControlsSubTab.Interface)
        {
            ...
            DrawCheckboxWithScroll(_soulMarkerGuidanceEnabled, sb);
            DrawCheckboxWithScroll(_flyingOutlineEnabled, sb);

            var uiFontBox = ScrollRect(_uiFontBox, scroll);
            var uiFontIndex = GetUiFontIndex(_working.UiFontStyle);
            DrawSimpleDropdownBox(sb, uiFontBox, "UI FONT", _uiFontOpen, UiFontLabels, uiFontIndex);
            ...
            if (_uiFontOpen)
                DrawSimpleDropdownList(sb, uiFontBox, UiFontLabels, uiFontIndex);
    ...
    private void DrawBindList(SpriteBatch sb, int scroll, IReadOnlyList<string> actions, bool drawExtendedNotes)
    {
        ...
        if (drawExtendedNotes)
        {
            ...
            var notes = new (string text, Color color)[]
            {
                ...
                ("HOME MENU: FAVORITE A HOME FOR CROUCH+HOME HOTKEY TELEPORT | SET HERE UPDATES COORDS", new Color(180, 180, 180)),
    ...
    private int GetAllBindsNoteLineCount()
    {
        ...
        var notes = new[]
        {
            ...
            "HOME MENU: FAVORITE A HOME FOR CROUCH+HOME TELEPORT | SET HERE UPDATES COORDS",
    ...
    private void DrawResolutionBox(SpriteBatch sb, Rectangle box)
    {
        ...
        var disabled = _working.Fullscreen;
        sb.Draw(_pixel, box, disabled ? new Color(14, 14, 14) : new Color(20, 20, 20));
        DrawBorder(sb, box, disabled ? new Color(110, 110, 110) : Color.White);

        var current = disabled ? "AUTO" : $"{_working.ResolutionWidth}x{_working.ResolutionHeight}";
        var textColor = disabled ? new Color(170, 170, 170) : Color.White;
        _font.DrawString(sb, current, new Vector2(box.X + 10, box.Y + 12), textColor);

        _font.DrawString(sb, disabled ? "-" : (_resolutionOpen ? "A" : "V"), new Vector2(box.Right - 40, box.Y + 12), textColor);
    }

    private void DrawResolutionList(SpriteBatch sb, Rectangle box)
    {
        if (_working.Fullscreen || !_resolutionOpen || _resList.Count == 0)
            return;
    ...
    private void UpdateUnsavedChangesModal(InputState input)
    {
        if (input.IsNewKeyPress(Keys.Escape))
        {
        ...
        if (input.ScrollDelta != 0 && _unsavedModalListRect.Contains(input.MousePosition))
        {
            var step = Math.Sign(input.ScrollDelta) * ScrollStep;
            var maxScroll = Math.Max(0, _unsavedChangesContentHeight - _unsavedModalListRect.Height);
            _unsavedChangesScroll = Math.Clamp(_unsavedChangesScroll - step, 0, maxScroll);
        }
        ...
        if (_unsavedStayRect.Contains(mouse))
        {
            _showUnsavedChangesModal = false;
            return;
        }

        if (_unsavedDiscardRect.Contains(mouse))
        {
            Cancel();
            StopMicTest();
            _menus.Pop();
        ...
        if (_unsavedApplyRect.Contains(mouse))
        {
            Apply();
            if (!_applyFeedbackIsError)
            {
                StopMicTest();
                _menus.Pop();
            }
        ...
        if (!_unsavedModalRect.Contains(mouse))
            _showUnsavedChangesModal = false;
    }

    private void DrawUnsavedChangesModal(SpriteBatch sb)
    {
        sb.Draw(_pixel, UiLayout.Viewport, new Color(0, 0, 0, 180));
        sb.Draw(_pixel, _unsavedModalRect, new Color(16, 22, 34, 245));
        DrawBorder(sb, _unsavedModalRect, new Color(188, 204, 232), 2);

        _font.DrawString(sb, "UNSAVED CHANGES", new Vector2(_unsavedModalRect.X + 26, _unsavedModalRect.Y + 20), Color.White);
        _font.DrawString(sb, "BACK WILL NOT APPLY THESE UNLESS YOU CHOOSE APPLY.", new Vector2(_unsavedModalRect.X + 26, _unsavedModalRect.Y + 52), new Color(220, 228, 240));

        sb.Draw(_pixel, _unsavedModalListRect, new Color(8, 12, 20, 220));
        DrawBorder(sb, _unsavedModalListRect, new Color(132, 148, 176), 1);

        var priorScissor = _graphics.GraphicsDevice.ScissorRectangle;
        _graphics.GraphicsDevice.ScissorRectangle = UiLayout.ToScreenRect(_unsavedModalListRect);
        sb.End();
        sb.Begin(samplerState: SamplerState.PointClamp, transformMatrix: UiLayout.Transform, rasterizerState: ScissorState);

        var y = _unsavedModalListRect.Y + 12 - (int)Math.Round(_unsavedChangesScroll);
        foreach (var change in _pendingSettingChanges)
        {
            var rowHeight = _font.LineHeight * 2 + 10;
            var rowRect = new Rectangle(_unsavedModalListRect.X + 10, y, _unsavedModalListRect.Width - 20, rowHeight);
            if (rowRect.Bottom >= _unsavedModalListRect.Y && rowRect.Y <= _unsavedModalListRect.Bottom)
            {
                sb.Draw(_pixel, rowRect, new Color(18, 26, 40, 220));
                DrawBorder(sb, rowRect, new Color(88, 104, 132), 1);
                _font.DrawString(sb, TrimToWidth(change.Label, rowRect.Width - 16), new Vector2(rowRect.X + 8, rowRect.Y + 6), Color.White);
                var detail = $"FROM {change.From} TO {change.To}";
                _font.DrawString(sb, TrimToWidth(detail, rowRect.Width - 16), new Vector2(rowRect.X + 8, rowRect.Y + 6 + _font.LineHeight), new Color(196, 214, 236));
            }
        ...
        sb.End();
        _graphics.GraphicsDevice.ScissorRectangle = priorScissor;
        sb.Begin(samplerState: SamplerState.PointClamp, transformMatrix: UiLayout.Transform);

        DrawUnsavedModalButton(sb, _unsavedStayRect, "STAY", new Color(34, 42, 58), new Color(170, 184, 212));
        DrawUnsavedModalButton(sb, _unsavedDiscardRect, "BACK ANYWAY", new Color(78, 40, 40), new Color(236, 166, 166));
        DrawUnsavedModalButton(sb, _unsavedApplyRect, "APPLY & BACK", new Color(34, 72, 48), new Color(164, 236, 182));
    }

    private void DrawUnsavedModalButton(SpriteBatch sb, Rectangle rect, string label, Color fill, Color border)
    {
        var hover = rect.Contains(_lastMouse);
        sb.Draw(_pixel, rect, hover ? Color.Lerp(fill, Color.White, 0.12f) : fill);
        DrawBorder(sb, rect, border, 2);
        var text = TrimToWidth(label, rect.Width - 10);
        var size = _font.MeasureString(text);
        _font.DrawString(sb, text, new Vector2(rect.Center.X - size.X / 2f, rect.Center.Y - _font.LineHeight / 2f), Color.White);
    }

    private List<SettingChangeEntry> BuildSettingChangeEntries()
    {
        var changes = new List<SettingChangeEntry>();

        AddChange(changes, "FULLSCREEN", FormatOnOff(_settings.Fullscreen), FormatOnOff(_working.Fullscreen));
        AddChange(changes, "VSYNC", FormatOnOff(_settings.VSync), FormatOnOff(_working.VSync));
        AddChange(changes, "RESOLUTION", FormatResolution(_settings), FormatResolution(_working));
        AddChange(changes, "GUI SCALE", FormatGuiScale(_settings.GuiScale), FormatGuiScale(_working.GuiScale));
        AddChange(changes, "QUALITY", NormalizeQuality(_settings.QualityPreset), NormalizeQuality(_working.QualityPreset));
        AddChange(changes, "BRIGHTNESS", $"{_settings.Brightness:0.00}", $"{_working.Brightness:0.00}");
        AddChange(changes, "FOV", _settings.FieldOfView.ToString(), _working.FieldOfView.ToString());
        AddChange(changes, "RENDER DISTANCE", $"{_settings.RenderDistanceChunks} CHUNKS", $"{_working.RenderDistanceChunks} CHUNKS");
        AddChange(changes, "PARTICLES", NormalizeParticlePreset(_settings.ParticlePreset), NormalizeParticlePreset(_working.ParticlePreset));

        AddChange(changes, "MASTER VOLUME", FormatPercent(_settings.MasterVolume), FormatPercent(_working.MasterVolume));
        AddChange(changes, "MUSIC VOLUME", FormatPercent(_settings.MusicVolume), FormatPercent(_working.MusicVolume));
        AddChange(changes, "SFX VOLUME", FormatPercent(_settings.SfxVolume), FormatPercent(_working.SfxVolume));
        AddChange(changes, "MULTISTREAM AUDIO", FormatOnOff(_settings.MultistreamAudio), FormatOnOff(_working.MultistreamAudio));
        AddChange(changes, "INPUT DEVICE", GetDeviceLabelById(_inputDevices, _settings.AudioInputDeviceId), GetDeviceLabelById(_inputDevices, _working.AudioInputDeviceId));
        AddChange(changes, "OUTPUT DEVICE", GetDeviceLabelById(_outputDevices, _settings.AudioOutputDeviceId), GetDeviceLabelById(_outputDevices, _working.AudioOutputDeviceId));
        AddChange(changes, "VOICE OUTPUT", GetDeviceLabelById(_outputDevices, _settings.VoiceOutputDeviceId), GetDeviceLabelById(_outputDevices, _working.VoiceOutputDeviceId));
        AddChange(changes, "GAME OUTPUT", GetDeviceLabelById(_outputDevices, _settings.GameOutputDeviceId), GetDeviceLabelById(_outputDevices, _working.GameOutputDeviceId));

        AddChange(changes, "MOUSE SENSITIVITY", $"{_settings.MouseSensitivity:0.0000}", $"{_working.MouseSensitivity:0.0000}");
        AddChange(changes, "TOGGLE CROUCH", FormatOnOff(_settings.ToggleCrouchEnabled), FormatOnOff(_working.ToggleCrouchEnabled));
        AddChange(changes, "SPRINT MODE", FormatSprintMode(_settings.SprintLatchEnabled), FormatSprintMode(_working.SprintLatchEnabled));
        AddChange(changes, "RETICLE", FormatOnOff(_settings.ReticleEnabled), FormatOnOff(_working.ReticleEnabled));
        AddChange(changes, "INDICATORS", FormatOnOff(_settings.IndicatorsEnabled), FormatOnOff(_working.IndicatorsEnabled));
        AddChange(changes, "SOUL MARKER GUIDE", FormatOnOff(_settings.SoulDeathMarkerGuidanceEnabled), FormatOnOff(_working.SoulDeathMarkerGuidanceEnabled));
        AddChange(changes, "FLY OUTLINE", FormatOnOff(_settings.FlyingOutlineEnabled), FormatOnOff(_working.FlyingOutlineEnabled));
        AddChange(changes, "UI FONT", GetUiFontLabel(_settings.UiFontStyle), GetUiFontLabel(_working.UiFontStyle));
        AddChange(changes, "RETICLE STYLE", NormalizeReticleStyleValue(_settings.ReticleStyle).ToUpperInvariant(), NormalizeReticleStyleValue(_working.ReticleStyle).ToUpperInvariant());
        AddChange(changes, "RETICLE SIZE", _settings.ReticleSize.ToString(), _working.ReticleSize.ToString());
        AddChange(changes, "RETICLE THICKNESS", _settings.ReticleThickness.ToString(), _working.ReticleThickness.ToString());
        AddChange(changes, "RETICLE COLOR", GetReticleColorLabel(_settings.ReticleColor), GetReticleColorLabel(_working.ReticleColor));
        AddChange(changes, "BLOCK OUTLINE COLOR", GetReticleColorLabel(_settings.BlockOutlineColor), GetReticleColorLabel(_working.BlockOutlineColor));
        AddChange(changes, "FLY OUTLINE COLOR", GetReticleColorLabel(_settings.FlyingOutlineColor), GetReticleColorLabel(_working.FlyingOutlineColor));
        AddChange(changes, "SOCIAL NOTIFICATIONS", GetNotificationModeLabel(_settings.GetSocialNotificationMode()), GetNotificationModeLabel(_working.GetSocialNotificationMode()));
        AddChange(changes, "NAMETAG MODE", GetNametagModeLabel(_settings.NametagMode), GetNametagModeLabel(_working.NametagMode));
        AddChange(changes, "NAMETAG FADE", $"{_settings.NametagFadeSeconds:0.0}S", $"{_working.NametagFadeSeconds:0.0}S");

        foreach (var action in _bindOrder)
        {
            var fromKey = _settings.Keybinds.TryGetValue(action, out var savedKey) ? savedKey.ToString().ToUpperInvariant() : "UNBOUND";
            var toKey = _working.Keybinds.TryGetValue(action, out var workingKey) ? workingKey.ToString().ToUpperInvariant() : "UNBOUND";
            AddChange(changes, GetBindActionLabel(action), fromKey, toKey);
        }

        var mouseActions = new HashSet<string>(_settings.MouseBinds.Keys, StringComparer.OrdinalIgnoreCase);
        foreach (var key in _working.MouseBinds.Keys)
            mouseActions.Add(key);
        foreach (var action in mouseActions.OrderBy(static x => x, StringComparer.OrdinalIgnoreCase))
        {
            var fromBind = _settings.MouseBinds.TryGetValue(action, out var savedMouse) ? savedMouse.ToUpperInvariant() : "UNBOUND";
            var toBind = _working.MouseBinds.TryGetValue(action, out var workingMouse) ? workingMouse.ToUpperInvariant() : "UNBOUND";
            AddChange(changes, $"{GetBindActionLabel(action)} MOUSE", fromBind, toBind);
        }

        return changes;
    }

    private static void AddChange(List<SettingChangeEntry> changes, string label, string from, string to)
    {
        if (string.Equals(from, to, StringComparison.Ordinal))
            return;

        changes.Add(new SettingChangeEntry
        {
            Label = label,
            From = from,
            To = to
    ...
    private static string FormatResolution(GameSettings settings) => $"{settings.ResolutionWidth}x{settings.ResolutionHeight}";
    private static string FormatGuiScale(float value)
    {
        var normalized = Math.Clamp(value, 1.0f, 2.0f);
        if (Math.Abs(normalized - 1.5f) < 0.01f)
            return "LARGE";
        if (Math.Abs(normalized - 2.0f) < 0.01f)
            return "LARGER";
        return "STANDARD";
    }
    private static string FormatPercent(float value) => $"{MathF.Round(Math.Clamp(value, 0f, 1f) * 100f):0}%";
    private static string FormatSprintMode(bool latch) => latch ? "AUTO / TOGGLE" : "HOLD";

    private string GetDeviceLabelById(List<DeviceOption> options, string id)
    {
        ...
        foreach (var option in options)
        {
            if (string.Equals(option.Id, id, StringComparison.OrdinalIgnoreCase))
                return option.Label.ToUpperInvariant();
        }

        return id.ToUpperInvariant();
    }

    private static string GetNotificationModeLabel(SocialNotificationMode mode)
    {
        foreach (var option in NotificationModeOptions)
        {
    ...
    private static string GetNametagModeLabel(string? mode)
    {
        return NametagModeLabels[Math.Clamp(GetNametagModeIndex(mode), 0, NametagModeLabels.Length - 1)];
    }

    private static string GetReticleColorLabel(string? value)
    {
        var normalized = NormalizeReticleColorValue(value);
        foreach (var option in ReticleColorOptions)
        {
    ...
    private static int GetUiFontIndex(string? value)
    {
        var normalized = GameSettings.NormalizeUiFontStyle(value);
        for (int i = 0; i < UiFontOptions.Length; i++)
        {
            if (string.Equals(UiFontOptions[i].value, normalized, StringComparison.OrdinalIgnoreCase))
                return i;
        }
    ...
    private static string GetUiFontLabel(string? value)
    {
        var index = GetUiFontIndex(value);
        return UiFontOptions[index].label;
    }
    ...
    private void Apply()
    {
        try
        {
            ...
            SnapWorkingVideoOptions();
            _working.UiFontStyle = GameSettings.NormalizeUiFontStyle(_working.UiFontStyle);
            _font.SetStyle(_working.UiFontStyle);
            // Persist
            _working.Save(_log);
            ...
            _working.CaptureActiveGraphics(_graphics);
            _working.Save(_log);
            CaptureVideoInfo();
            var device = _graphics.GraphicsDevice;
            if (device != null)
            ...
            _showUnsavedChangesModal = false;
            _pendingSettingChanges.Clear();
            _unsavedChangesScroll = 0f;
        }
        catch (Exception ex)
        {
    ...
    private void Cancel()
    {
        ...
        _soulMarkerGuidanceEnabled.Value = _working.SoulDeathMarkerGuidanceEnabled;
    ...
    private void ExitOptions()
    {
        ...
        CloseAllDropdowns();
        _showMultistreamHelp = false;
        _pendingSettingChanges = BuildSettingChangeEntries();
        if (_pendingSettingChanges.Count == 0)
        {
            _menus.Pop();
            return;
        }

        _unsavedChangesContentHeight = _pendingSettingChanges.Count * (_font.LineHeight * 2 + 10);
        _unsavedChangesScroll = 0f;
        _showUnsavedChangesModal = true;
    }

    private void RestoreSavedRuntimeSettings()
    {
        try
        {
            _settings = GameSettings.LoadOrCreate(_log);
            ...
            {
                var windowViewport = new Rectangle(0, 0, device.Viewport.Width, device.Viewport.Height);
                UiLayout.Update(windowViewport, UiLayout.GetEffectiveScale(_settings.GuiScale));
                _menus.OnResize(UiLayout.Viewport);
            }
            CaptureVideoInfo();
        }
        ...
        {
            _log.Warn($"Failed to restore saved runtime settings: {ex.Message}");
        }
    }
    ...
    private void CaptureVideoInfo()
    {
        try
        {
            ...
            var monitor = DisplayMonitorLocator.GetForWindow(_graphics.GraphicsDevice?.PresentationParameters.DeviceWindowHandle ?? IntPtr.Zero);
            var deviceName = monitor.DeviceName;
            var desktop = GetDesktopResolution(deviceName, mode.Width, mode.Height);
            var resolutionLimit = _working.Fullscreen
                ? monitor.Bounds
            ...
            var monitorLabel = SanitizeForFont(string.IsNullOrWhiteSpace(deviceName) ? "UNKNOWN" : deviceName);
            _displayInfo = $"DISPLAY: {desktop.w}x{desktop.h} {monitorLabel} {format}";

            var profile = _graphics.GraphicsDevice?.GraphicsProfile.ToString() ?? "UNKNOWN";
            _profileInfo = "PROFILE: " + SanitizeForFont(profile);

            BuildResolutionList(deviceName, Math.Max(640, resolutionLimit.Width), Math.Max(360, resolutionLimit.Height));
        }
        catch (Exception ex)
        {
    ...
    public void SyncExternalFullscreenState(bool fullscreen)
    {
        _working.Fullscreen = fullscreen;
    ...
    private void BuildResolutionList(string? deviceName, int desktopW, int desktopH)
    {
        ...
        {
            if (!IsValidResolutionOption(w, h))
                continue;
        ...
        if (IsValidResolutionOption(desktopW, desktopH))
            set.Add((desktopW, desktopH));
        if (IsValidResolutionOption(_working.ResolutionWidth, _working.ResolutionHeight)
            && _working.ResolutionWidth <= desktopW
            && _working.ResolutionHeight <= desktopH)
            set.Add((_working.ResolutionWidth, _working.ResolutionHeight));
    ...
    private void BuildResolutionFallback(int desktopW, int desktopH)
    {
        ...
        {
            if (IsValidResolutionOption(w, h) && w <= desktopW && h <= desktopH)
                _resList.Add((w, h));
        }
        ...
        if (IsValidResolutionOption(_working.ResolutionWidth, _working.ResolutionHeight)
            && _working.ResolutionWidth <= desktopW
            && _working.ResolutionHeight <= desktopH)
            _resList.Add((_working.ResolutionWidth, _working.ResolutionHeight));

        if (_resList.Count == 0 && IsValidResolutionOption(desktopW, desktopH))
    ...
    private static bool IsValidResolutionOption(int width, int height)
