...
public sealed class MoreWorldOptionsScreen : IScreen
{
    ...
    private readonly Action<bool, bool, bool, bool, string, bool, int, bool, bool> _onSettingsChanged;
    // (generateOres, generateCaves, generateStructures, generateTrees, worldType, enableHomes, homeSlots, playerCollision, pvpEnabled)
    ...
    private readonly Button _pvpBtn;
    ...
    private bool _homeSlotsInputSelectAll;
    ...
    public MoreWorldOptionsScreen(
        ...
        bool pvpEnabled,
        int maxHomesCap,
        Action<bool, bool, bool, bool, string, bool, int, bool, bool> onSettingsChanged)
    {
        _menus = menus;
        _assets = assets;
        _font = font;
        ...
        _pvpBtn = new Button(string.Empty, TogglePvp);
    ...
    public void OnResize(Rectangle viewport)
    {
        ...
        gY += 56;
        _pvpBtn.Bounds = new Rectangle(buttonX, gY, buttonW, buttonH);
        gY += 62;
    ...
    public void Update(GameTime gameTime, InputState input)
    {
        ...
        else if (_activeTab == Tab.Gameplay)
        {
            if (input.IsNewLeftClick())
                TextFieldVisuals.HandleWholeFieldPointerSelection(input, _homeSlotsInputRect, ref _homeSlotsInputActive, ref _homeSlotsInputSelectAll, _homeSlotsInputText, _enableHomes);
    ...
    private void DrawHomeSlotsInput(SpriteBatch sb)
    {
        ...
        TextFieldVisuals.DrawWholeFieldSelection(sb, _pixel, _font, _homeSlotsInputText, textPos, _homeSlotsInputRect, _homeSlotsInputSelectAll, new Color(88, 148, 218, 170));
        _font.DrawString(sb, displayText, textPos, textColor);
        TextFieldVisuals.DrawCaret(sb, _pixel, _font, _homeSlotsInputText, textPos, _homeSlotsInputRect, _homeSlotsInputActive && _enableHomes, _homeSlotsInputSelectAll, _now, Color.White);
    }
    ...
    private void HandleHomeSlotsInput(InputState input)
    {
        ...
        if (ctrl)
        {
            ...
            {
                _homeSlotsInputSelectAll = true;
                return;
            }

            if (input.IsNewKeyPress(Keys.C))
            {
                if (!string.IsNullOrEmpty(_homeSlotsInputText))
                    WinClipboard.SetText(_homeSlotsInputText);
                return;
            }
            ...
            if (input.IsNewKeyPress(Keys.X))
            {
                if (!string.IsNullOrEmpty(_homeSlotsInputText))
                    WinClipboard.SetText(_homeSlotsInputText);
                _homeSlotsInputText = string.Empty;
                _homeSlotsInputSelectAll = false;
                return;
            }

            if (input.IsNewKeyPress(Keys.V))
            {
                var clip = new string((WinClipboard.GetText() ?? string.Empty).Where(char.IsDigit).ToArray());
                if (_homeSlotsInputSelectAll)
                    _homeSlotsInputText = string.Empty;
                if (_homeSlotsInputText.Length < 10)
                    _homeSlotsInputText += clip[..Math.Min(clip.Length, 10 - _homeSlotsInputText.Length)];
                _homeSlotsInputSelectAll = false;
                return;
            }
        }
        ...
        foreach (var key in input.GetTextInputKeys())
        {
            if (key == Keys.Back)
            {
                ...
                else if (_homeSlotsInputText.Length > 0)
    ...
    private void SyncButtonLabels()
    {
        ...
        _pvpBtn.Label = _pvpEnabled ? "PVP: Enabled" : "PVP: Disabled";
    }
