...
public sealed class MultiplayerHostScreen : IScreen
{
    ...
    private void DeleteSelectedWorld()
    {
        ...
        _log.Info($"Delete world requested: {entry.Name}");
    }
    ...
    private void HostSelectedWorld()
    {
        ...
        if (_hostForFriends)
        {
            _log.Info($"HOST_CLICKED world={entry.Name} mode=online_screen transport=online");
    ...
    private void DrawDeleteConfirmOverlay(SpriteBatch sb)
    {
        ...
        var worldName = (_deleteConfirmIndex >= 0 && _deleteConfirmIndex < _worlds.Count)
            ...
            : "UNKNOWN WORLD";
        var body = TruncateToWidth(worldName, _deleteConfirmRect.Width - 36);
