...
public sealed class ScreenshotsScreen : IScreen
{
    ...
    private Rectangle _modalRect;
    private Rectangle _modalMessageRect;
    private Rectangle _modalPrimaryRect;
    private Rectangle _modalSecondaryRect;
    private bool _showModal;
    private bool _modalIsConfirm;
    private string _modalTitle = string.Empty;
    private string _modalMessage = string.Empty;
    private string _modalPrimaryLabel = "OK";
    private string _modalSecondaryLabel = "CANCEL";
    ...
    public void OnResize(Rectangle viewport)
    {
        ...
        var modalWidth = Math.Min(640, viewport.Width - 80);
        var modalHeight = 220;
        _modalRect = new Rectangle(
            viewport.Center.X - modalWidth / 2,
            viewport.Center.Y - modalHeight / 2,
            modalWidth,
            modalHeight);
        _modalMessageRect = new Rectangle(_modalRect.X + 24, _modalRect.Y + 68, _modalRect.Width - 48, 72);
        var buttonWidth = Math.Min(220, (_modalRect.Width - 72) / (_modalIsConfirm ? 2 : 1));
        var buttonHeight = 44;
        var buttonY = _modalRect.Bottom - buttonHeight - 22;
        _modalPrimaryRect = _modalIsConfirm
            ? new Rectangle(_modalRect.Center.X + 8, buttonY, buttonWidth, buttonHeight)
            : new Rectangle(_modalRect.Center.X - buttonWidth / 2, buttonY, buttonWidth, buttonHeight);
        _modalSecondaryRect = new Rectangle(_modalRect.Center.X - buttonWidth - 8, buttonY, buttonWidth, buttonHeight);
    ...
    private void DeleteSelectedScreenshot()
    {
        ...
        {
            ShowModal("SCREENSHOTS", "SELECT A SCREENSHOT TO DELETE FIRST.", false, "OK", null, null);
            return;
        }
        ...
        ShowModal(
            "DELETE SCREENSHOT",
            $"DELETE THIS SCREENSHOT?\n{Path.GetFileName(item.Path).ToUpperInvariant()}",
            true,
            "DELETE",
            PerformDeleteSelectedScreenshot,
            "CANCEL");
    }

    private void PerformDeleteSelectedScreenshot()
    {
        try
        {
            if (_selectedIndex < 0 || _selectedIndex >= _items.Count)
        ...
        catch (Exception ex)
        {
            ...
            ShowModal("SCREENSHOTS", "FAILED TO DELETE SCREENSHOT.", false, "OK", null, null);
        }
    }
    ...
    private void ShowModal(string title, string message, bool confirm, string primaryLabel, Action? primaryAction, string? secondaryLabel)
    {
        _showModal = true;
        _modalTitle = title;
        _modalMessage = message;
        _modalIsConfirm = confirm;
        _modalPrimaryLabel = primaryLabel;
        _modalSecondaryLabel = string.IsNullOrWhiteSpace(secondaryLabel) ? "CANCEL" : secondaryLabel!;
        _modalPrimaryAction = primaryAction;
        OnResize(_viewport);
    }

    private void UpdateModal(InputState input)
    {
        if (input.IsNewKeyPress(Keys.Escape))
        {
            _showModal = false;
        ...
        if (_modalPrimaryRect.Contains(mouse))
        {
        ...
        if (_modalIsConfirm && _modalSecondaryRect.Contains(mouse))
        {
            _showModal = false;
            _modalPrimaryAction = null;
            return;
        ...
        if (!_modalRect.Contains(mouse) && !_modalIsConfirm)
        {
            _showModal = false;
            _modalPrimaryAction = null;
        }
    ...
    private void DrawModal(SpriteBatch sb)
    {
        sb.Draw(_pixel, UiLayout.Viewport, new Color(0, 0, 0, 180));
        sb.Draw(_pixel, _modalRect, new Color(16, 22, 34, 245));
        DrawBorder(sb, _modalRect, new Color(188, 204, 232), 2);

        _font.DrawString(sb, _modalTitle, new Vector2(_modalRect.X + 24, _modalRect.Y + 20), Color.White);

        var lines = _modalMessage.Split('\n', StringSplitOptions.RemoveEmptyEntries | StringSplitOptions.TrimEntries);
        var y = _modalMessageRect.Y;
        foreach (var line in lines)
        {
            _font.DrawString(sb, TrimToWidth(line, _modalMessageRect.Width), new Vector2(_modalMessageRect.X, y), new Color(226, 234, 245));
            y += _font.LineHeight + 4;
        }

        DrawModalButton(sb, _modalPrimaryRect, _modalPrimaryLabel, new Color(76, 40, 40), new Color(236, 166, 166));
        if (_modalIsConfirm)
            DrawModalButton(sb, _modalSecondaryRect, _modalSecondaryLabel, new Color(34, 42, 58), new Color(170, 184, 212));
    }

    private void DrawModalButton(SpriteBatch sb, Rectangle rect, string label, Color fill, Color border)
    {
        ...
        var text = TrimToWidth(label, rect.Width - 12);
