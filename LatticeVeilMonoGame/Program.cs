...
public static class Program
{
    ...
    public static void Main(string[] args)
    {
        ...
        AppStateMigration.MigrateToRoaming(log);
        EosSecretHygiene.PurgeLocalSecrets(log);
        ...
        var startupSkinImportClipboard = LauncherProtocolLinking.HasSkinImportClipboardRequest(args);
        var startupSkinLibraryRefresh = LauncherProtocolLinking.HasSkinLibraryRefreshRequest(args);
        ...
        else if (startupSkinImportClipboard)
            log.Info("Launcher skin import request detected from protocol URL.");
        else if (startupSkinLibraryRefresh)
            log.Info("Launcher skin library refresh request detected from protocol URL.");
        ...
        if (runGame)
        {
            ...
            RuntimeSessionCache.DeleteLegacyDocumentsCachesOnly(log);
        ...
        if (!createdNew)
        {
            if (!string.IsNullOrWhiteSpace(startupLinkCode))
            {
                ...
                {
                    LauncherProtocolLinking.TryFocusRunningLauncher(log);
                    return;
                }
            }
            else if (startupSkinImportClipboard)
            {
                if (LauncherProtocolLinking.TryQueueSkinImportClipboardForRunningLauncher(log))
                {
                    LauncherProtocolLinking.TryFocusRunningLauncher(log);
                    return;
                }
            ...
            {
                if (LauncherProtocolLinking.TryQueueSkinLibraryRefreshForRunningLauncher(log))
                {
                    LauncherProtocolLinking.TryFocusRunningLauncher(log);
                    return;
                }
            ...
            LauncherProtocolLinking.TryQueueRestoreLauncherForRunningLauncher(log);
            LauncherProtocolLinking.TryFocusRunningLauncher(log);
            return;
        }
        ...
        try
        {
            RuntimeSessionCache.ResetSocialSession(log);
            LauncherProtocolLinking.TryEnsureProtocolRegistration(log);
            Application.EnableVisualStyles();
            Application.SetCompatibleTextRenderingDefault(false);
            Application.Run(new LauncherForm(log, profile, startupLinkCode, startupSkinImportClipboard, startupSkinLibraryRefresh));
