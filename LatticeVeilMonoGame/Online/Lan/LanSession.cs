...
public readonly struct LanWorldInfo
{
    ...
    public bool SaveInventory { get; init; }
    public bool SharedSoulAccess { get; init; }
    public bool PvpEnabled { get; init; }
    public string DeathType { get; init; }
...
public readonly struct LanPlayerState
{
    ...
    public float SigilAtonement { get; init; }
    public float SigilCurse { get; init; }
}
...
public enum LanPlayerCombatKind : byte
{
    Request = 0,
    Result = 1
}

public readonly struct LanPlayerCombat
{
    public LanPlayerCombatKind Kind { get; init; }
    public int AttackerPlayerId { get; init; }
    public int TargetPlayerId { get; init; }
    public int Damage { get; init; }
    public int TargetHealth { get; init; }
    public bool TargetDied { get; init; }
    public float KnockbackX { get; init; }
    public float KnockbackY { get; init; }
    public float KnockbackZ { get; init; }
}
...
public interface ILanSession : IDisposable
{
    ...
    void SendPlayerState(Vector3 position, float yaw, float pitch, byte status, byte heldBlockId, Vector3 velocity, int health, int hunger, float sigilAtonement, float sigilCurse);
    ...
    bool SendPlayerCombat(LanPlayerCombat combat);
    ...
    bool TryDequeuePlayerCombat(out LanPlayerCombat combat);
...
public sealed class LanHostSession : ILanSession
{
        ...
        lock (_clientLock)
        {
            foreach (var client in _clients.Values)
            {
                if (client.Id == normalized.PlayerId)
                    continue;
                LanWire.SendPersistenceSnapshot(client, normalized);
            }
        }
    }
    ...
    public bool SendPlayerCombat(LanPlayerCombat combat)
    {
        ...
        lock (_clientLock)
            _clients.TryGetValue(combat.TargetPlayerId, out target);

        if (target == null)
            return false;

        LanWire.SendPlayerCombat(target, combat);
        return true;
    }
    ...
    public bool TryDequeuePlayerCombat(out LanPlayerCombat combat) => _combatMessages.TryDequeue(out combat);
            ...
            while (!token.IsCancellationRequested)
            {
                ...
                switch (msg.Value.Type)
                {
                    ...
                    case LanMessageType.PlayerCombat:
                        var combat = msg.Value.PlayerCombat;
                        _combatMessages.Enqueue(new LanPlayerCombat
                        {
                            Kind = combat.Kind,
                            AttackerPlayerId = client.Id,
                            TargetPlayerId = combat.TargetPlayerId,
                            Damage = combat.Damage,
                            TargetHealth = combat.TargetHealth,
                            TargetDied = combat.TargetDied,
                            KnockbackX = combat.KnockbackX,
                            KnockbackY = combat.KnockbackY,
                            KnockbackZ = combat.KnockbackZ
                        });
                        break;
                }
            }
    ...
    private void BroadcastPlayerList()
    {
        lock (_clientLock)
        {
            ...
            {
                new LanPlayerInfo { PlayerId = LocalPlayerId, Name = _hostName, AuthId = string.Empty }
            };

            foreach (var client in _clients.Values)
            {
                var name = string.IsNullOrWhiteSpace(client.Name) ? $"Player{client.Id}" : client.Name;
                players.Add(new LanPlayerInfo { PlayerId = client.Id, Name = name, AuthId = string.Empty });
            }
    ...
    private static class LanWire
    {
        ...
        public static void SendPlayerCombat(ClientState client, LanPlayerCombat combat)
        {
            Write(client, bw =>
            {
            ...
            while (!token.IsCancellationRequested)
            {
                ...
                switch (msg.Value.Type)
                {
                    ...
                    case LanMessageType.InventoryView:
                        ...
                        break;
                    case LanMessageType.PlayerCombat:
                        _combatMessages.Enqueue(msg.Value.PlayerCombat);
                        break;
...
public readonly struct LanMessage
{
    ...
    public LanPlayerCombat PlayerCombat { get; init; }
