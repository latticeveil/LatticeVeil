...
internal sealed class VeilnetProfileClient
{
    ...
    private async Task<VeilnetProfileFetchResult> GetFromEndpointAsync(string endpoint, string accessToken, CancellationToken ct)
    {
        ...
        _log?.Info(
            $"[VeilnetProfileClient] request: method=GET endpoint={endpointPath} " +
        ...
        try
        {
            ...
            _log?.Info($"[VeilnetProfileClient] response: status={(int)response.StatusCode} {SummarizeProfileBodyForLog(body)}");
    ...
    private static string SummarizeProfileBodyForLog(string? body)
    {
        ...
        try
        {
            using var doc = JsonDocument.Parse(value, new JsonDocumentOptions { AllowTrailingCommas = true });
            var root = doc.RootElement;
            if (root.ValueKind != JsonValueKind.Object)
                return $"bodyType={root.ValueKind}";

            var profileObj = root;
            if (root.TryGetProperty("profile", out var nested) && nested.ValueKind == JsonValueKind.Object)
                profileObj = nested;

            var ok = root.TryGetProperty("ok", out var okProp) && (okProp.ValueKind == JsonValueKind.True || okProp.ValueKind == JsonValueKind.False)
                ? $"ok={okProp.GetBoolean()} "
                : string.Empty;
            var usernamePresent = profileObj.TryGetProperty("username", out var usernameProp)
                && usernameProp.ValueKind == JsonValueKind.String
                && !string.IsNullOrWhiteSpace(usernameProp.GetString());
            var picturePresent = profileObj.TryGetProperty("picture", out var pictureProp)
                && pictureProp.ValueKind == JsonValueKind.String
                && !string.IsNullOrWhiteSpace(pictureProp.GetString());
            var bannerPresent = profileObj.TryGetProperty("banner", out var bannerProp)
                && bannerProp.ValueKind == JsonValueKind.String
                && !string.IsNullOrWhiteSpace(bannerProp.GetString());
            var aboutPresent = profileObj.TryGetProperty("about_me", out var aboutSnake)
                && aboutSnake.ValueKind == JsonValueKind.String
                && !string.IsNullOrWhiteSpace(aboutSnake.GetString());
            if (!aboutPresent)
                aboutPresent = profileObj.TryGetProperty("aboutMe", out var aboutCamel)
                    && aboutCamel.ValueKind == JsonValueKind.String
                    && !string.IsNullOrWhiteSpace(aboutCamel.GetString());

            return $"{ok}usernamePresent={usernamePresent} picturePresent={picturePresent} bannerPresent={bannerPresent} aboutPresent={aboutPresent}";
        }
        catch
        {
            return $"bodyText={ClipForLog(value, 80)}";
        }
    }
