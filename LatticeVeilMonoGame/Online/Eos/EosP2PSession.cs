...
public sealed class EosP2PHostSession : ILanSession
{
    ...
    private readonly ConcurrentQueue<LanPlayerCombat> _combatMessages = new();
    ...
    private static string ShortPeerId(object? value)
    {
    ...
    public void PreApprovePuid(string puid)
    {
        ...
        lock (_clientLock)
        {
            ...
            {
                _log.Info($"PUID {ShortPeerId(puid)} is now socially pre-approved for P2P join.");
            }

            // If they are already waiting in the native P2P queue, approve them NOW
            if (_pendingJoinByPeer.ContainsKey(puid.Trim()))
            {
                _log.Info($"PUID {ShortPeerId(puid)} was already waiting for P2P approval; auto-approving now.");
    ...
    public void SendChat(LanChatMessage message)
    {
        ...
        if (normalized.ToPlayerId < 0 || normalized.ToPlayerId == LocalPlayerId)
    ...
    public void SendPersistenceSnapshot(LanPlayerPersistenceSnapshot snapshot)
    {
        ...
        if (IsHost)
        {
            ...
            lock (_clientLock)
            {
                ...
                {
                    if (client.Id == normalized.PlayerId)
                        continue;
                    EosP2PWire.SendPersistenceSnapshot(_p2p, _localUserId, client.PeerId, _socketId, normalized, _log);
                }
            }
        }
#else
    ...
    public void SendPlayerState(Vector3 position, float yaw, float pitch, byte status, byte heldBlockId, Vector3 velocity, int health, int hunger, float sigilAtonement, float sigilCurse)
    {
#if EOS_SDK
        if (!IsConnected)
            return;
        ...
        var state = new LanPlayerState
        {
            ...
            Health = health,
            Hunger = hunger,
            SigilAtonement = sigilAtonement,
            SigilCurse = sigilCurse
        };
    ...
    {
#if EOS_SDK
        while (_pendingJoinQueue.TryDequeue(out request))
        {
            var key = (request.ProductUserId ?? string.Empty).Trim();
    ...
    public bool TryDequeuePlayerCombat(out LanPlayerCombat combat)
    {
#if EOS_SDK
        return _combatMessages.TryDequeue(out combat);
#else
        combat = default;
        return false;
#endif
    ...
    public bool SendPlayerCombat(LanPlayerCombat combat)
    {
#if EOS_SDK
        if (!IsConnected || combat.TargetPlayerId <= 0)
            return false;
        ...
        lock (_clientLock)
        {
            foreach (var client in _clients.Values)
            {
                if (client.Id != combat.TargetPlayerId)
                    continue;
                target = client;
                break;
            }
        ...
        EosP2PWire.SendPlayerCombat(_p2p, _localUserId, target.PeerId, _socketId, combat, _log);
        return true;
#else
        return false;
#endif
    ...
    public bool ApproveJoinRequest(string peerProductUserId)
    {
        ...
        AnnouncePlayerJoined(approved);
        _log.Info($"EOS join request approved ({ShortPeerId(pending!.Key)}).");
        BroadcastPlayerList();
    ...
    public bool DeclineJoinRequest(string peerProductUserId, string reason = "Host declined join request.")
    {
        ...
        _log.Info($"EOS join request declined ({ShortPeerId(pending!.Key)}).");
                   ...
                   EosP2PWire.TryReceivePacket(_p2p, _localUserId, _socketId, out var peerId, out var msg))
            {
                ...
                switch (msg.Type)
                {
                    ...
                    case LanMessageType.InventoryView:
                        ...
                        break;
                    case LanMessageType.PlayerCombat:
                        if (!TryGetClient(peerId, out var combatSender))
                            break;
                        _combatMessages.Enqueue(new LanPlayerCombat
                        {
                            Kind = msg.PlayerCombat.Kind,
                            AttackerPlayerId = combatSender.Id,
                            TargetPlayerId = msg.PlayerCombat.TargetPlayerId,
                            Damage = msg.PlayerCombat.Damage,
                            TargetHealth = msg.PlayerCombat.TargetHealth,
                            TargetDied = msg.PlayerCombat.TargetDied,
                            KnockbackX = msg.PlayerCombat.KnockbackX,
                            KnockbackY = msg.PlayerCombat.KnockbackY,
                            KnockbackZ = msg.PlayerCombat.KnockbackZ
                        });
                        break;
                }
            }
    ...
    private void HandleHello(ProductUserId peerId, string? name, string? gateTicket)
    {
        if (TryGetClient(peerId, out var approvedClient))
        {
            ...
            AnnouncePlayerJoined(approvedClient);
            return;
        }
        ...
        _onlineGate.TryExtractPeerTicketUserId(gateTicket, out var invitedUserId);
        lock (_clientLock)
        {
            if (_sociallyApprovedPuids.Contains(key)
                || (!string.IsNullOrWhiteSpace(invitedUserId) && _sociallyApprovedPuids.Contains(invitedUserId)))
            {
                ...
                _log.Info($"EOS P2P: Auto-approving pre-approved peer {ShortPeerId(key)}.");
            ...
            if (!_pendingJoinByPeer.TryGetValue(key, out pending))
            {
                ...
                _log.Info($"EOS join request queued ({ShortPeerId(pending.Key)}, namePresent={!string.IsNullOrWhiteSpace(pending.Name)}).");
            }
            else if (!string.IsNullOrWhiteSpace(name))
            {
    ...
    private bool TryAuthorizePeer(ProductUserId peerId, string? gateTicket)
    {
        ...
        _log.Warn($"EOS peer rejected by gate validation ({ShortPeerId(peerId)}): {denialReason}");
    ...
    private void ClosePeerConnection(ProductUserId peerId)
    {
        try
        {
            ...
            if (result != Result.Success)
                _log.Warn($"EOS close peer failed for {ShortPeerId(peerId)}: {result}");
        }
        catch (Exception ex)
        {
            _log.Warn($"EOS close peer exception for {ShortPeerId(peerId)}: {ex.Message}");
        }
    }
    ...
    private void AnnouncePlayerJoined(ClientState client)
    {
        if (client.JoinAnnounced)
            return;

        client.JoinAnnounced = true;
        var name = string.IsNullOrWhiteSpace(client.Name) ? $"Player{client.Id}" : client.Name.Trim();
        var message = new LanChatMessage
        {
            FromPlayerId = LocalPlayerId,
            ToPlayerId = -1,
        ...
        lock (_clientLock)
        {
            foreach (var peer in _clients.Values)
            {
                ...
                EosP2PWire.SendChat(_p2p, _localUserId, peer.PeerId, _socketId, message, _log);
            }
        }
    }
    ...
    private void BroadcastPlayerList()
    {
        lock (_clientLock)
        {
            ...
            {
                new LanPlayerInfo { PlayerId = LocalPlayerId, Name = _hostName, AuthId = _localUserId.ToString() }
            };

            foreach (var client in _clients.Values)
            {
                var name = string.IsNullOrWhiteSpace(client.Name) ? $"Player{client.Id}" : client.Name;
                players.Add(new LanPlayerInfo { PlayerId = client.Id, Name = name, AuthId = client.PeerId.ToString() });
            }
    ...
    private void RemoveClient(ProductUserId peerId)
    {
        ...
        {
            _log.Info($"EOS pending join request removed ({ShortPeerId(removedPending.Key)}).");
        }
    }
#endif
    ...
    public void Dispose()
    {
        ...
        var shutdownReason = "HOST LEFT THE GAME";
        lock (_clientLock)
        {
            foreach (var client in _clients.Values)
            {
...
public sealed class EosP2PClientSession : ILanSession
{
        ...
        TimeSpan timeout)
    {
        ...
        try
        {
            ...
            var nextHelloResend = DateTime.UtcNow + TimeSpan.FromSeconds(1);
            var deadline = DateTime.UtcNow + timeout;
            log.Info($"EOS_JOIN_REQUEST sent host={ShortPeerId(hostId)} timeout={timeout.TotalSeconds:0}s");
            ...
            while (DateTime.UtcNow < deadline)
            {
                ...
                if (msg.Type == LanMessageType.JoinDenied)
                {
                    var reason = string.IsNullOrWhiteSpace(msg.JoinDeniedReason)
                        ...
                        : msg.JoinDeniedReason!;
                    log.Info($"EOS_JOIN_REQUEST_RESULT host={ShortPeerId(hostId)} status=declined");
                ...
                {
                    log.Info($"EOS_JOIN_REQUEST_RESULT host={ShortPeerId(hostId)} status=approved");
                    return new EosJoinApprovalResult { Status = EosJoinApprovalStatus.Approved, Message = "approved" };
                }
            }

            log.Info($"EOS_JOIN_REQUEST_RESULT host={ShortPeerId(hostId)} status=pending");
            return new EosJoinApprovalResult
            {
    ...
    public bool SendPlayerCombat(LanPlayerCombat combat)
    {
        ...
        EosP2PWire.SendPlayerCombat(_p2p, _localUserId, _hostUserId, _socketId, combat, _log);
        return true;
#else
        return false;
#endif
    ...
    private void OnConnectionClosed(ref OnRemoteConnectionClosedInfo info)
    {
        ...
        {
            SetDisconnectReasonOnce("HOST LEFT THE GAME");
            IsConnected = false;
        }
    }
                   ...
                   EosP2PWire.TryReceivePacket(_p2p, _localUserId, _socketId, out var peerId, out var msg))
            {
                ...
                switch (msg.Type)
                {
                    ...
                    case LanMessageType.WorldSyncComplete:
                        ...
                        break;
                    case LanMessageType.PersistenceSnapshot:
                        _persistenceSnapshots.Enqueue(msg.PersistenceSnapshot);
                        break;
                    ...
                    case LanMessageType.InventoryView:
                        ...
                        break;
                    case LanMessageType.PlayerCombat:
                        _combatMessages.Enqueue(msg.PlayerCombat);
                        break;
...
internal static class EosP2PWire
{
    ...
    public static void SendWorldInfo(P2PInterface p2p, ProductUserId localUserId, ProductUserId remoteUserId, SocketId socketId, LanWorldInfo info, Logger log)
    {
        var payload = BuildPayload(bw =>
        {
            ...
            bw.Write(info.SaveInventory);
            bw.Write(info.SharedSoulAccess);
            bw.Write(info.PvpEnabled);
            bw.Write(string.IsNullOrWhiteSpace(info.DeathType) ? GameplaySettings.DefaultDeathType : info.DeathType);
    ...
    public static void SendPlayerState(P2PInterface p2p, ProductUserId localUserId, ProductUserId remoteUserId, SocketId socketId, LanPlayerState state, Logger log)
    {
        var payload = BuildPayload(bw =>
        {
            ...
            bw.Write(state.VelocityX);
            bw.Write(state.VelocityY);
            bw.Write(state.VelocityZ);
            bw.Write(state.Health);
            bw.Write(state.Hunger);
            bw.Write(state.SigilAtonement);
            bw.Write(state.SigilCurse);
        }, log);
    ...
    public static void SendPlayerCombat(P2PInterface p2p, ProductUserId localUserId, ProductUserId remoteUserId, SocketId socketId, LanPlayerCombat combat, Logger log)
    {
        var payload = BuildPayload(bw =>
        {
            bw.Write((byte)LanMessageType.PlayerCombat);
            bw.Write((byte)combat.Kind);
            bw.Write(combat.AttackerPlayerId);
            bw.Write(combat.TargetPlayerId);
            bw.Write(combat.Damage);
            bw.Write(combat.TargetHealth);
            bw.Write(combat.TargetDied);
            bw.Write(combat.KnockbackX);
            bw.Write(combat.KnockbackY);
            bw.Write(combat.KnockbackZ);
        }, log);
        SendRaw(p2p, localUserId, remoteUserId, socketId, payload, PacketReliability.ReliableOrdered, log);
    }
    ...
    public static void SendPlayerList(P2PInterface p2p, ProductUserId localUserId, ProductUserId remoteUserId, SocketId socketId, LanPlayerList list, Logger log)
    {
        var payload = BuildPayload(bw =>
        {
            ...
            for (var i = 0; i < count; i++)
            {
                ...
                bw.Write(list.Players[i].AuthId ?? string.Empty);
            }
        }, log);
        SendRaw(p2p, localUserId, remoteUserId, socketId, payload, PacketReliability.ReliableOrdered, log);
    }
    ...
    private static bool TryParseMessage(byte[] packet, out LanMessage msg)
    {
        ...
        switch (type)
        {
            ...
            case LanMessageType.WorldInfo:
            {
                ...
                var saveInventory = br.BaseStream.Position < br.BaseStream.Length ? br.ReadBoolean() : false;
                var sharedSoulAccess = br.BaseStream.Position < br.BaseStream.Length ? br.ReadBoolean() : false;
                var pvpEnabled = br.BaseStream.Position < br.BaseStream.Length ? br.ReadBoolean() : false;
                var deathType = br.BaseStream.Position < br.BaseStream.Length ? br.ReadString() : GameplaySettings.DefaultDeathType;
                ...
                msg = new LanMessage
                {
                    ...
                    WorldInfo = new LanWorldInfo
                    {
                        ...
                        DeathType = string.IsNullOrWhiteSpace(deathType) ? GameplaySettings.DefaultDeathType : deathType.Trim().ToLowerInvariant(),
                ...
                msg = new LanMessage
                {
                    ...
                    PlayerState = new LanPlayerState
                    {
                        ...
                        HeldBlockId = br.BaseStream.Position < br.BaseStream.Length
                            ? br.ReadByte()
                            : (byte)BlockId.Air,
                        VelocityX = br.BaseStream.Position + sizeof(float) <= br.BaseStream.Length
                            ? br.ReadSingle()
                            : 0f,
                        VelocityY = br.BaseStream.Position + sizeof(float) <= br.BaseStream.Length
                            ? br.ReadSingle()
                            : 0f,
                        VelocityZ = br.BaseStream.Position + sizeof(float) <= br.BaseStream.Length
                            ? br.ReadSingle()
                            : 0f,
                        Health = br.BaseStream.Position + sizeof(int) <= br.BaseStream.Length
                            ? br.ReadInt32()
                            : SurvivalVitals.MaxHealth,
                        Hunger = br.BaseStream.Position + sizeof(int) <= br.BaseStream.Length
                            ? br.ReadInt32()
                            : SurvivalVitals.MaxHunger,
                        SigilAtonement = br.BaseStream.Position + sizeof(float) <= br.BaseStream.Length
                            ? br.ReadSingle()
                            : 0f,
                        SigilCurse = br.BaseStream.Position + sizeof(float) <= br.BaseStream.Length
                            ? br.ReadSingle()
                            : 0f
                    }
                };
            ...
            case LanMessageType.PlayerList:
                ...
                for (var i = 0; i < count; i++)
                {
                    players[i] = new LanPlayerInfo
                    {
                        PlayerId = br.ReadInt32(),
                        Name = br.ReadString(),
                        AuthId = br.BaseStream.Position < br.BaseStream.Length ? br.ReadString() : string.Empty
                    };
                }
            ...
            case LanMessageType.PlayerCombat:
                ...
                {
                    Type = type,
                    PlayerCombat = new LanPlayerCombat
                    {
                        Kind = (LanPlayerCombatKind)br.ReadByte(),
                        AttackerPlayerId = br.ReadInt32(),
                        TargetPlayerId = br.ReadInt32(),
                        Damage = br.ReadInt32(),
                        TargetHealth = br.ReadInt32(),
                        TargetDied = br.ReadBoolean(),
                        KnockbackX = br.ReadSingle(),
                        KnockbackY = br.ReadSingle(),
                        KnockbackZ = br.ReadSingle()
                    }
                };
                return true;
            case LanMessageType.HostShutdown:
