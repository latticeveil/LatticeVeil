...
public sealed class EosClient : IDisposable
{
    ...
    private const string LobbyAttrWhitelist = "rc_whitelist";
    private const string LobbyAttrWorldId = "rc_world_id";
    private const string LobbyAttrHeartbeatUtc = "rc_heartbeat_utc";
    ...
    private EosClient(Logger log, EosConfig config)
    {
        _log = log;
#if EOS_SDK
        _config = config;
...
#else
    public string? LocalProductUserId => null;
    public string? EpicAccountId => null;
    public string? EpicDisplayName => null;
    ...
    public bool IsLoggedIn
    {
        ...
        {
#if EOS_SDK
            lock (_sdkLock) return _localUserId != null && _localUserId.IsValid();
#else
            return false;
#endif
        }
    ...
    public bool IsEpicLoggedIn
    {
        ...
        {
#if EOS_SDK
            lock (_sdkLock) return _epicAccountId != null && _epicAccountId.IsValid();
#else
            return false;
#endif
        }
    ...
    {
#if EOS_SDK
        lock (_sdkLock) { if (!_disposed) _platform?.Tick(); }
#endif
    }
    ...
    {
#if EOS_SDK
        if (IsLoggedIn || _deviceLoginStarted) return;
        BeginLogin();
#endif
    }
    ...
    private bool Initialize()
    {
        ...
        try
        {
            ...
            EosSecretHygiene.ClearSecretEnvironment();
        ...
        string? whitelistNames = null)
    {
        ...
        var normalizedWhitelistNames = NormalizeOptional(whitelistNames, 512);
        ...
        var updated = await UpdateHostedLobbyStateAsync(
            ...
            normalizedWorldId,
            normalizedWhitelistNames).ConfigureAwait(false);
        ...
        string whitelistNames)
    {
        ...
        try
        {
            ...
            if (!AddOrReplaceLobbyAttribute(mod, LobbyAttrMaxPlayers, maxPlayers))
                return false;
            var heartbeatUnix = (int)Math.Clamp(DateTimeOffset.UtcNow.ToUnixTimeSeconds(), int.MinValue, int.MaxValue);
            if (!AddOrReplaceLobbyAttribute(mod, LobbyAttrHeartbeatUtc, heartbeatUnix))
                return false;
            ...
            if (!string.IsNullOrWhiteSpace(whitelistNames))
            {
                if (!AddOrReplaceLobbyAttribute(mod, LobbyAttrWhitelist, whitelistNames))
                    return false;
            }
    ...
    private bool TryReadHostedLobbyEntry(LobbyDetails details, out EosHostedLobbyEntry entry)
    {
        ...
        var whitelistNames = CopyLobbyStringAttribute(details, LobbyAttrWhitelist, string.Empty);
        var worldId = CopyLobbyStringAttribute(details, LobbyAttrWorldId, string.Empty);
        var heartbeatUnix = CopyLobbyIntAttribute(details, LobbyAttrHeartbeatUtc, 0);
        var lastHeartbeatUtc = heartbeatUnix > 0
            ? DateTimeOffset.FromUnixTimeSeconds(heartbeatUnix).UtcDateTime
            : DateTime.UtcNow;
    ...
    private static string NormalizeOptional(string? value, int maxLength = 64)
    {
...
public sealed class EosHostedLobbyEntry
{
    ...
    public string WhitelistNames { get; init; } = string.Empty;
