...
public static class EosRemoteConfigBootstrap
{
    ...
    private static bool TryFetchPublic(Logger log, string? ticket, out EosConfigPayload payload)
    {
        ...
        try
        {
            ...
            {
                log.Warn($"EOS remote config request failed: HTTP {(int)response.StatusCode} - {TrimSnippet(body, 220)}");
                return false;
            }
    ...
    private static void ApplyPublicEnvironment(EosConfigPayload payload)
    {
        ...
        Environment.SetEnvironmentVariable("EOS_PRODUCT_VERSION", string.IsNullOrWhiteSpace(payload.ProductVersion) ? "10.0.0" : payload.ProductVersion);
