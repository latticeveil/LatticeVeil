...
public sealed class PlayerWorldState
{
    ...
    public string FavoriteHomeName { get; set; } = string.Empty;
    ...
    public bool HasSoulMarker { get; set; }
    public float SoulMarkerX { get; set; }
    public float SoulMarkerY { get; set; }
    public float SoulMarkerZ { get; set; }
    public float SoulMarkerYaw { get; set; }
    public long SoulMarkerSpawnedUtcTicks { get; set; }
    public long SoulMarkerExpireUtcTicks { get; set; }
    public string SoulOwnerUsername { get; set; } = "";
    public string SoulOwnerPlayerId { get; set; } = "";
    public HotbarSlot[] SoulInventory { get; set; } = new HotbarSlot[Inventory.HotbarSize + Inventory.GridSize];
            ...
            try
            {
                ...
                if (data.Count > 0)
                {
                    ...
                    if (float.TryParse(data.GetValueOrDefault("homeZ"), out var homeZ))
                        state.HomeZ = homeZ;
                    state.FavoriteHomeName = data.GetValueOrDefault("favoriteHomeName") ?? string.Empty;
                    ...
                    if (bool.TryParse(data.GetValueOrDefault("hasSoulMarker"), out var hasSoulMarker))
                        state.HasSoulMarker = hasSoulMarker;
                    if (float.TryParse(data.GetValueOrDefault("soulMarkerX"), out var soulX))
                        state.SoulMarkerX = soulX;
                    if (float.TryParse(data.GetValueOrDefault("soulMarkerY"), out var soulY))
                        state.SoulMarkerY = soulY;
                    if (float.TryParse(data.GetValueOrDefault("soulMarkerZ"), out var soulZ))
                        state.SoulMarkerZ = soulZ;
                    if (float.TryParse(data.GetValueOrDefault("soulMarkerYaw"), out var soulYaw))
                        state.SoulMarkerYaw = soulYaw;
                    if (long.TryParse(data.GetValueOrDefault("soulMarkerSpawnedUtcTicks"), out var soulSpawnedUtcTicks))
                        state.SoulMarkerSpawnedUtcTicks = soulSpawnedUtcTicks;
                    if (long.TryParse(data.GetValueOrDefault("soulMarkerExpireUtcTicks"), out var soulExpireUtcTicks))
                        state.SoulMarkerExpireUtcTicks = soulExpireUtcTicks;
                    state.SoulOwnerUsername = data.GetValueOrDefault("soulOwnerUsername") ?? string.Empty;
                    state.SoulOwnerPlayerId = data.GetValueOrDefault("soulOwnerPlayerId") ?? string.Empty;
                    if (int.TryParse(data.GetValueOrDefault("soulInventoryCount"), out var soulInventoryCount) && soulInventoryCount > 0)
                    {
                        state.SoulInventory = new HotbarSlot[soulInventoryCount];
                        for (int i = 0; i < soulInventoryCount; i++)
                        {
                            var id = BlockId.Air;
                            var count = 0;
                            if (int.TryParse(data.GetValueOrDefault($"soul.{i}.id"), out var soulBlockId))
                                id = (BlockId)soulBlockId;
                            if (int.TryParse(data.GetValueOrDefault($"soul.{i}.count"), out var soulCount))
                                count = soulCount;

                            state.SoulInventory[i] = new HotbarSlot { Id = id, Count = count };
                        }
                    }
                    ...
                    if (int.TryParse(data.GetValueOrDefault("homesCount"), out var homesCount) && homesCount > 0)
                    {
                        ...
                        for (int i = 0; i < homesCount; i++)
                        {
                            var home = new PlayerHomeState
                            {
                                ...
                                HomeColorPacked = uint.TryParse(data.GetValueOrDefault($"home.{i}.colorPacked"), out var colorPacked) ? colorPacked : 0u
                            };
    ...
    public void Save(string worldPath, string saveKey, Logger log)
    {
        try
        {
            ...
            var data = new Dictionary<string, string>
            {
                ...
                ["favoriteHomeName"] = FavoriteHomeName ?? string.Empty,
                ...
                ["hasSoulMarker"] = HasSoulMarker.ToString(),
                ["soulMarkerX"] = SoulMarkerX.ToString("F6"),
                ["soulMarkerY"] = SoulMarkerY.ToString("F6"),
                ["soulMarkerZ"] = SoulMarkerZ.ToString("F6"),
                ["soulMarkerYaw"] = SoulMarkerYaw.ToString("F6"),
                ["soulMarkerSpawnedUtcTicks"] = SoulMarkerSpawnedUtcTicks.ToString(),
                ["soulMarkerExpireUtcTicks"] = SoulMarkerExpireUtcTicks.ToString(),
                ["soulOwnerUsername"] = SoulOwnerUsername ?? string.Empty,
                ["soulOwnerPlayerId"] = SoulOwnerPlayerId ?? string.Empty
            };
            ...
            if (Homes != null && Homes.Count > 0)
            {
                ...
                for (int i = 0; i < Homes.Count; i++)
                {
                    ...
                    data[$"home.{i}.colorPacked"] = home.HomeColorPacked.ToString();
                }
            }
            ...
            {
                data["soulInventoryCount"] = SoulInventory.Length.ToString();
                for (int i = 0; i < SoulInventory.Length; i++)
                {
                    var slot = SoulInventory[i];
                    data[$"soul.{i}.id"] = ((int)slot.Id).ToString();
                    data[$"soul.{i}.count"] = slot.Count.ToString();
                }
            }
    ...
    private void WriteBinaryPayload(BinaryWriter bw)
    {
        ...
        bw.Write(HomeY);
        bw.Write(HomeZ);
        bw.Write(FavoriteHomeName ?? string.Empty);
        ...
        bw.Write(HasSoulMarker);
        bw.Write(SoulMarkerX);
        bw.Write(SoulMarkerY);
        bw.Write(SoulMarkerZ);
        bw.Write(SoulMarkerYaw);
        bw.Write(SoulMarkerSpawnedUtcTicks);
        bw.Write(SoulMarkerExpireUtcTicks);
        bw.Write(SoulOwnerUsername ?? string.Empty);
        bw.Write(SoulOwnerPlayerId ?? string.Empty);

        var soulSlots = SoulInventory ?? Array.Empty<HotbarSlot>();
        var soulCount = Math.Min(soulSlots.Length, Inventory.HotbarSize + Inventory.GridSize);
        bw.Write((byte)soulCount);
        for (int i = 0; i < soulCount; i++)
        {
            bw.Write((byte)soulSlots[i].Id);
            bw.Write(soulSlots[i].Count);
        }
    ...
    private static bool TryReadBinaryPayload(BinaryReader br, Logger? log, out PlayerWorldState state)
    {
        ...
        try
        {
            ...
            if (version >= 14)
                state.FavoriteHomeName = br.ReadString();
            ...
            if (version >= 10)
            {
                state.HasSoulMarker = br.ReadBoolean();
                state.SoulMarkerX = br.ReadSingle();
                state.SoulMarkerY = br.ReadSingle();
                state.SoulMarkerZ = br.ReadSingle();
                state.SoulMarkerYaw = br.ReadSingle();
                state.SoulMarkerSpawnedUtcTicks = br.ReadInt64();
                state.SoulMarkerExpireUtcTicks = br.ReadInt64();
            }
            if (version >= 11)
            {
                if (version >= 12)
                {
                    state.SoulOwnerUsername = br.ReadString();
                    state.SoulOwnerPlayerId = br.ReadString();
                }

                var soulCount = br.ReadByte();
                state.SoulInventory = new HotbarSlot[Inventory.HotbarSize + Inventory.GridSize];
                for (int i = 0; i < Math.Min((int)soulCount, state.SoulInventory.Length); i++)
                    state.SoulInventory[i] = new HotbarSlot { Id = (BlockId)br.ReadByte(), Count = br.ReadInt32() };
            }
            ...
            for (int i = 0; i < homeCount; i++)
            {
                var home = new PlayerHomeState
                {
                    ...
                    HomeColorPacked = version >= 13 ? br.ReadUInt32() : 0u
                };
                state.Homes.Add(home);
            }
...
public sealed class PlayerHomeState
{
    ...
    public uint HomeColorPacked { get; set; }
}
