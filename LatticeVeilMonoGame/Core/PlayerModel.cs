...
public sealed class PlayerModel : IDisposable
{
    ...
    private const float SkinLayerInflate = 0.026f;

    private readonly GraphicsDevice _device;
    private readonly Logger? _log;
    private readonly bool _watchLocalSkin;
    ...
    public PlayerModel(GraphicsDevice device, AssetLoader? assets = null, Logger? log = null, bool watchLocalSkin = true)
    {
        _device = device;
        _log = log;
        _watchLocalSkin = watchLocalSkin;
        ...
        if (_watchLocalSkin && PlayerSkinSource.TryLoadTexture(device, out var skinTexture, out var skinPath, out var writeUtc))
        {
            _skinTexture = skinTexture;
            _ownsSkinTexture = true;
            _skinFilePath = skinPath;
            _skinFileWriteUtc = writeUtc;
            _log?.Info($"Player skin loaded: path={skinPath}");
        }
        else
        {
    ...
    public bool TrySetSkinFromPngBytes(byte[] pngBytes, string? sourceLabel = null)
    {
        try
        {
            var texture = PlayerSkinSource.LoadCompositedTexture(_device, pngBytes);
            SetSkinTexture(texture, takeOwnership: true);
            _skinFilePath = null;
            _skinFileWriteUtc = DateTime.MinValue;
            _log?.Info($"Player skin texture applied from bytes{(string.IsNullOrWhiteSpace(sourceLabel) ? string.Empty : $": {sourceLabel}")}");
            return true;
        }
        catch (Exception ex)
        {
            _log?.Warn($"Player skin texture apply failed{(string.IsNullOrWhiteSpace(sourceLabel) ? string.Empty : $" ({sourceLabel})")}: {ex.Message}");
            return false;
        }
    }

    public void ResetSkinToDefault(string? reason = null)
    {
        SetSkinTexture(DefaultPlayerSkinFactory.Create(_device), takeOwnership: true);
        _skinFilePath = null;
        _skinFileWriteUtc = DateTime.MinValue;
        _log?.Info($"Player skin reset to default{(string.IsNullOrWhiteSpace(reason) ? string.Empty : $": {reason}")}");
    }

    public void RefreshWatchedSkin()
    {
    ...
    public void ResetAnimationState()
    {
        ...
        _estimatedMoveSpeed = 0f;
        _estimatedVerticalSpeed = 0f;
        _walkAmountSmoothed = 0f;
        _moveCyclePhase = 0f;
        _actionSwingTimer = 0f;
        _actionSwingAimYawOffset = 0f;
        _actionSwingAimPitch = 0f;
        _hasPoseCache = false;
    }

    public void Render(Matrix view, Matrix projection, float worldTimeSeconds)
    {
        Render(view, projection, worldTimeSeconds, Color.White, 1f, true);
    }

    public void RenderHead(Matrix view, Matrix projection, float worldTimeSeconds, Color tint, float alpha)
    {
        RenderInternal(view, projection, worldTimeSeconds, tint, alpha, drawHeldBlock: false, headOnly: true);
    }

    public void Render(Matrix view, Matrix projection, float worldTimeSeconds, Color tint, float alpha, bool drawHeldBlock)
    {
        RenderInternal(view, projection, worldTimeSeconds, tint, alpha, drawHeldBlock, headOnly: false);
    }

    private void RenderInternal(Matrix view, Matrix projection, float worldTimeSeconds, Color tint, float alpha, bool drawHeldBlock, bool headOnly)
    {
        _hasPoseCache = false;
        ...
        _baseEffect.Alpha = Math.Clamp(alpha, 0f, 1f);
        _baseEffect.DiffuseColor = tint.ToVector3();

        _overlayEffect.View = view;
        _overlayEffect.Projection = projection;
        _overlayEffect.Texture = _skinTexture;
        _overlayEffect.Alpha = Math.Clamp(alpha, 0f, 1f);
        ...
        _bodyWorld = BuildPartWorld(_body, root, new Vector3(0f, 1.07f + crouchOffset + upperBodyBob, sneakForwardShift), bodyRot, Vector3.Zero);
        if (!headOnly)
            DrawPart(_body, _bodyWorld);
        ...
        _headWorld = BuildPartWorld(_head, root, new Vector3(0f, 1.59f + crouchOffset + upperBodyBob, sneakForwardShift * 0.7f), headRot, new Vector3(0f, -_head.HalfSize.Y + 0.03f, 0f));
        DrawPart(_head, _headWorld);

        if (headOnly)
        {
            _bodyWorld = Matrix.Identity;
            _leftArmWorld = Matrix.Identity;
            _rightArmWorld = Matrix.Identity;
            _leftLegWorld = Matrix.Identity;
            _rightLegWorld = Matrix.Identity;
            _hasPoseCache = true;
            return;
        }
        ...
        _leftArmWorld = BuildPartWorld(_leftArm, root, new Vector3(-0.38f, armBaseY, sneakForwardShift), leftArmRot, armPivot);
        _rightArmWorld = BuildPartWorld(_rightArm, root, new Vector3(0.38f, armBaseY, sneakForwardShift), rightArmRot, armPivot);
        DrawPart(_leftArm, _leftArmWorld);
        DrawPart(_rightArm, _rightArmWorld);
        if (drawHeldBlock)
        ...
        _leftLegWorld = BuildPartWorld(_leftLeg, root, new Vector3(-legOffsetX, legBaseY, 0f), leftLegRot, legPivot);
        _rightLegWorld = BuildPartWorld(_rightLeg, root, new Vector3(legOffsetX, legBaseY, 0f), rightLegRot, legPivot);
        DrawPart(_leftLeg, _leftLegWorld);
        DrawPart(_rightLeg, _rightLegWorld);
        _hasPoseCache = true;
    }
    ...
    private static Matrix BuildPartWorld(PartMesh part, Matrix root, Vector3 partCenter, Matrix localRotation, Vector3 pivot)
    {
        return BuildPartWorld(root, partCenter, localRotation, pivot);
    }

    private void DrawPart(PartMesh part, Matrix world)
    {
    ...
    private void TryHotReloadSkinFromFile()
    {
        ...
        if (pathChanged)
        {
            ...
            _skinFileWriteUtc = DateTime.MinValue;
            _log?.Info($"Player skin path changed: path={resolvedPath}");
            if (string.IsNullOrWhiteSpace(_skinFilePath))
            {
                ResetSkinToDefault("active skin cleared");
                return;
            }
        }
        ...
        try
        {
            var reloaded = PlayerSkinSource.LoadCompositedTexture(_device, File.ReadAllBytes(_skinFilePath));
            SetSkinTexture(reloaded, takeOwnership: true);
            _skinFileWriteUtc = writeUtc;
            _log?.Info($"Player skin hot-reloaded: path={_skinFilePath}");
        }
        catch (Exception ex)
        {
            _log?.Debug($"Player skin hot-reload skipped: {ex.Message}");
        ...
        float overlayInflate)
    {
        ...
        var overlayVerts = BuildTexturedBox(half + new Vector3(SkinLayerInflate), overlayMap);
