...
public sealed class Logger
{
    ...
    private static readonly Regex SensitiveAssignmentRegex = new(
        @"(?i)\b(?<key>Authorization|x-gate-ticket|gate[_-]?ticket|access[_-]?token|refresh[_-]?token|apikey|anon[_-]?key|service[_-]?role[_-]?key|client[_-]?secret|EOS_CLIENT_SECRET|SUPABASE_ANON_KEY|SUPABASE_SERVICE_ROLE_KEY|LV_VEILNET_ACCESS_TOKEN)\b\s*[:=]\s*(?<value>[^\s,;]+)",
        RegexOptions.Compiled);
    private static readonly Regex SensitiveIdRegex = new(
        @"(?i)\b(?<key>hostId|peerId|puid|productUserId|lobbyId)\b\s*=\s*(?<value>[A-Za-z0-9\-_:]{12,})",
        RegexOptions.Compiled);
    ...
    private void Write(string level, string message, string memberName, string sourceFilePath, int sourceLineNumber)
    {
        ...
        var line = $"{DateTime.Now:HH:mm:ss.fff} [{level}] [T:{threadId}]{location} {sanitizedMessage}";
    ...
    private static string Sanitize(string? message)
    {
        ...
        sanitized = SensitiveAssignmentRegex.Replace(sanitized, m => $"{m.Groups["key"].Value}=[redacted]");
        sanitized = SensitiveIdRegex.Replace(sanitized, m => $"{m.Groups["key"].Value}=[redacted]");
        return sanitized;
    }
