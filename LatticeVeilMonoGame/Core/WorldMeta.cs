...
public sealed class WorldMeta
{
    ...
    public List<string> OperatorIdentityIds { get; set; } = new();
    ...
    public static WorldMeta CreateFlat(string name, GameMode mode, int width, int height, int depth, int seed)
    {
        return new WorldMeta
        {
            ...
            OperatorIdentityIds = new List<string>(),
            ...
            Gameplay = new GameplaySettings
            {
                ...
                SaveInventory = false,
                SharedSoulAccess = false,
                EnablePvp = false,
                DeathType = GameplaySettings.DefaultDeathType,
                EnableMultipleHomes = true,
    ...
    {
        Gameplay.DeathType = string.IsNullOrWhiteSpace(Gameplay.DeathType)
            ? GameplaySettings.DefaultDeathType
            : Gameplay.DeathType.Trim().ToLowerInvariant();
        ...
        OperatorIdentityIds = Gameplay.OperatorIdentityIds ?? OperatorIdentityIds;
    ...
    private void ApplyLegacyFieldsToGroupedSettings()
    {
        ...
        Gameplay.EnablePvp = Gameplay.EnablePvp;
        Gameplay.DeathType = string.IsNullOrWhiteSpace(Gameplay.DeathType)
            ? GameplaySettings.DefaultDeathType
            : Gameplay.DeathType.Trim().ToLowerInvariant();
        ...
        Gameplay.OperatorIdentityIds = OperatorIdentityIds ?? Gameplay.OperatorIdentityIds;
    ...
    private void WriteSectionedWorldManifest(string path)
    {
        var gameplayEntries = new List<KeyValuePair<string, string>>
        {
            ...
            Kv("save_inventory", Gameplay.SaveInventory ? "true" : "false"),
            Kv("shared_soul_access", Gameplay.SharedSoulAccess ? "true" : "false"),
            Kv("pvp", Gameplay.EnablePvp ? "true" : "false"),
            Kv("death_type", string.IsNullOrWhiteSpace(Gameplay.DeathType) ? GameplaySettings.DefaultDeathType : Gameplay.DeathType),
        ...
        var operatorIdsValue = BuildOperatorsValue(Gameplay.OperatorIdentityIds);
        if (!string.IsNullOrWhiteSpace(operatorIdsValue))
            gameplayEntries.Add(Kv("operator_ids", operatorIdsValue));
    ...
    private static string NormalizeManifestKey(string? section, string key)
    {
        ...
        return section.Trim().ToUpperInvariant() switch
        {
            ...
            "GAMEPLAY" => normalizedKey switch
            {
                ...
                "save_inventory" => "Gameplay.SaveInventory",
                "shared_soul_access" => "Gameplay.SharedSoulAccess",
                "pvp" => "Gameplay.EnablePvp",
                "death_type" => "Gameplay.DeathType",
                ...
                "operator_ids" => "Gameplay.OperatorIdentityIds",
                _ => $"Gameplay.{key}"
            },
