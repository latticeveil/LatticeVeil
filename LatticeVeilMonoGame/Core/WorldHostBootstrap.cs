...
public static class WorldHostBootstrap
{
        ...
        bool isInWorld)
    {
        ...
        try
        {
            ...
            if (hosting)
            {
                ...
                var mode = (meta?.CurrentWorldGameMode ?? GameMode.Artificer).ToString();
                ...
                ok = await eos.StartOrUpdateHostedLobbyAsync(
                    worldName,
                    mode,
                    cheats: meta?.EnableCheats ?? false,
                    playerCount: 1,
                    maxPlayers: 8,
                    isInWorld: isInWorld,
    ...
    private static LanWorldInfo BuildWorldInfo(WorldMeta meta)
    {
        return new LanWorldInfo
        {
            ...
            SaveInventory = meta.Gameplay?.SaveInventory ?? false,
            SharedSoulAccess = meta.Gameplay?.SharedSoulAccess ?? false,
            PvpEnabled = meta.Gameplay?.EnablePvp ?? false,
            DeathType = string.IsNullOrWhiteSpace(meta.Gameplay?.DeathType) ? GameplaySettings.DefaultDeathType : meta.Gameplay.DeathType.Trim().ToLowerInvariant(),
    ...
    private sealed class LanDiscoveryHostSession : ILanSession
    {
        ...
        public void SendPlayerState(Vector3 position, float yaw, float pitch, byte status, byte heldBlockId, Vector3 velocity, int health, int hunger, float sigilAtonement, float sigilCurse) => _inner.SendPlayerState(position, yaw, pitch, status, heldBlockId, velocity, health, hunger, sigilAtonement, sigilCurse);
        ...
        public bool SendPlayerCombat(LanPlayerCombat combat) => _inner.SendPlayerCombat(combat);
        ...
        public bool TryDequeuePlayerCombat(out LanPlayerCombat combat) => _inner.TryDequeuePlayerCombat(out combat);
