...
public sealed class InputState
{
    ...
    public bool IsLeftDragActive(int minimumPixels = 3)
    {
        if (_mouse.LeftButton != global::Microsoft.Xna.Framework.Input.ButtonState.Pressed
            || _prevMouse.LeftButton != global::Microsoft.Xna.Framework.Input.ButtonState.Pressed)
        {
            return false;
        }

        var dx = _mouse.X - _prevMouse.X;
        var dy = _mouse.Y - _prevMouse.Y;
        return (dx * dx) + (dy * dy) >= minimumPixels * minimumPixels;
    }
